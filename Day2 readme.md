...
- Configured Splunk to listen and receive incoming log data on **port 9997** via Settings → Forwarding and Receiving → Configure Receiving → Add new → 9997
...
> The Splunk home screen showing Search & Reporting, Dashboards, Alerts, and Reports on the left sidebar — the main interface used throughout this project to search logs and build detections.
...
- Sysmon accepted the EULA and started as a background Windows service — confirmed with "Sysmon64 started" message
...
- Verified network connectivity from VM to host using PowerShell:

```powershell
Test-NetConnection -ComputerName 192.168.119.1 -Port 9997
```
...
**Why 192.168.119.1:** The Windows 11 VM uses NAT networking in VMware. The VMware VMnet8 virtual adapter on the host gets assigned `192.168.119.1` as the gateway — this is the address the VM uses to reach the host machine, and therefore where it sends its logs.
...
**Key Sysmon Event IDs observed in first batch:**
